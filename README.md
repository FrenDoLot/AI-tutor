...
- Передача изображений и документов в Xiaomi MiMo как файлов, а не только как имён.
...
- Xiaomi MiMo API через OpenAI-compatible API и SDK `openai`
- Pydantic
- python-dotenv
...
- `backend/app/main.py` — API, авторизация, чаты, Xiaomi MiMo provider, подписки, лимиты, настройки.
...
- `MIMO_MAX_COMPLETION_TOKENS` — максимальное количество токенов ответа.
- `MIMO_THINKING` — включает reasoning/thinking, если модель поддерживает этот параметр.
- `MIMO_STREAM` — включает streaming на backend. Ответ собирается сервером и возвращается прежним API.
...
- формирует prompt для Xiaomi MiMo;
- передаёт в MiMo текст, историю текущего чата и прикреплённые файлы.
...
2. Если лимит исчерпан, сообщение не сохраняется и MiMo не вызывается.
...
9. Backend отправляет prompt и файлы в Xiaomi MiMo.
10. Backend сохраняет ответ AI.
11. Backend возвращает актуальный чат.

Контекст MiMo включает только историю текущего чата. История других чатов и других пользователей никогда не используется.
...
Xiaomi MiMo получает:

- текст пользователя;
- историю текущего чата;
- описание вложений;
- изображения как OpenAI-compatible `image_url` data URL;
- текстовые файлы как текст;
- документы как OpenAI-compatible file content part, с fallback на текстовое описание, если провайдер не принял бинарный document part.
...
- Не отправлять MiMo только имя изображения, если пользователь прикрепил изображение.
